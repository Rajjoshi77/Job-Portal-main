...
export default function Intro_jobseeker() {
  ...
  const handleSearch = (e) => {
    ...
    const filteredJobs = jobData?.filter((job) => {
      const categoryMatch = job?.job_category?.toUpperCase().includes(search?.toUpperCase().trim());
      const pinMatch = job?.pin_number === parseInt(search.trim(), 10);
      const Salary = job?.salary
      return categoryMatch || pinMatch || Salary;
    });
  ...
  return (
    <>
        ...
        <div className='lg:w-3/6 w-full sm:p-2 h-full my-2 flex items-center justify-center px-4 md:items-start md:justify-start md:p-20 flex-col '>
          ...
          <div className=' w-full px-2 py-2 flex items-center justify-start flex-wrap'>
            ...
            <div className='flex   items-center justify-center px-4 flex-wrap'>
              ...
              <p className='px-2  text-gray-600'>Digital Marketing</p>
      ...
      <a target="_blank" href="https://api.whatsapp.com/send?phone=+919426823235&text=Hello%20HireSphere%Job %20%20I%20Portal%20Looking%20For%20">
        <img src="/wlogo.png" className="pulse" style={{ height: 'auto', width: 100, background: 'none', position: 'fixed', borderRadius: 35, bottom: 0, margin: '0 0 10px 10px', zIndex: 9999 }} />
      </a>
