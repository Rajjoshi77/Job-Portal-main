...
export default function Login() {
  ...
  const [formData, setFormData] = useState({ email: "", password: "", jType: "" });
  const [error, setError] = useState({ email: "", password: "", jType: "" });
  ...
  const handleSubmit = async (e) => {
    ...
    if (res.success) {
      ...
      if (formData.jType === 'company') {
        Router.push('/frontend/dashboard');
      } else if (formData.jType === 'jobseeker') {
        Router.push('/frontend_jobseeker/displayJobs');
      } else if (formData.jType === 'admin') {
          ...
          <div className="w-full bg-white text-black rounded-lg shadow dark:border md:mt-0 sm:max-w-md xl:p-0 ">
            <div className="p-6 space-y-4 md:space-y-6 sm:p-8">
              ...
              <form onSubmit={handleSubmit} className="space-y-4 md:space-y-6" action="#">
                ...
                <div className="text-left">
                  <label htmlFor="userType" className="block mb-2 text-sm font-medium text-gray-900">Select User Type</label>
                  <select
                    onChange={(e) => setFormData({ ...formData, jType: e.target.value })}
                  ...
                  {error.jType && <p className="text-sm text-red-500">{error.jType}</p>}
                </div>
